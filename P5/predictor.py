...
- Get the return over X periods which is the label to predict. This needs a forward buffer
- Augment data with bollinger band (rolling mean and std). This needs a back buffer
- Adjust Open, High and Low like Close was adjusted with Adjusted Close
- Normalize by the max value. Needs to be fitted so later data can be normalized the same way
...
class SVRPredictor(object):
    ...
    def fit(self, start_date, end_date):
        ...
        for ticker, stock in self.stocks.items():
            ...
            # X_train, X_cv, y_train, y_cv = train_test_split(data, label) # TODO fit_transform stock only on X_train and not on X_cv
...
class Stock(object):
    ...
    def get_data(self, start_date, end_date, fit=False, cache=True):
        ...
        if cache and os.path.exists(d_file_name) and os.path.exists(l_file_name):
            ...
            label = pd.read_csv(
                l_file_name, index_col=0, parse_dates=True, squeeze=True, header=None)
        else:
            data = QuandlAPI.get_data(self.ticker, start_date, end_date, 15, 15)
            ...
            sp500 = QuandlAPI.get_data(
                "INDEX_GSPC", start_date, end_date, 15, 15)
        ...
        # So two subsequent call with two different period will be adjusted differently
        ...
        data = self.rescale.transform(data[start_date:end_date])
